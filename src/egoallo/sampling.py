...
def quadratic_ts(return_tensor = False, set_final_step: bool = True) -> np.ndarray | torch.Tensor:
    ...
    if set_final_step:
        x[-1] = start_step
    if return_tensor:
        return torch.from_numpy(np.ascontiguousarray(x[::-1])).to(torch.long)
    else:
        return x[::-1]
...
def run_sampling_with_logprob(
    ...
    x_t_packed_list = [x_t_packed] # Packed tensor list
    ...
    for i in range(len(ts) - 1): # Denoising loop
        t = ts[i] # 1000
        t_next = ts[i + 1] # 841
        ...
        if guidance_mode != "off" and guidance_inner:
            ...
            for b in range(batch_size):
                x_0_pred, _ = do_guidance_optimization(
                    ...
                    traj=network.EgoDenoiseTraj.unpack(
                        x_0_packed_pred[b], include_hands=denoiser_network.config.include_hands
                    ),
                    body_model=body_model,
                    guidance_mode=guidance_mode,
                    phase="inner",
                ...
                x_0_pred_list.append(x_0_pred.pack())
            x_0_packed_pred = torch.stack(x_0_pred_list, dim=0)
            del x_0_pred_list
        ...
        # DDIM process || x_t_packed is the x_{t-1}
        x_t_packed_wonoise = (
            torch.sqrt(alpha_bar_t[t_next]) * x_0_packed_pred
            + (
                torch.sqrt(1 - alpha_bar_t[t_next] - sigma_t[t] ** 2)
